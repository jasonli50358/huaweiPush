...
class PushConfig
{
    ...
    private function __construct()
    {
        if ($this->log_read_flag == false) {
            $huaweiPushConfig = \Yii::$app->params['huaweiPush'];
            // read line
            $this->log_read_flag = true;
            $this->HW_APPID = $huaweiPushConfig['APP_ID'];
            $this->HW_APPSECRET = $huaweiPushConfig['APP_SECRET'];
            $this->HW_TOKEN_SERVER = "https://oauth-login.cloud.huawei.com/oauth2/v2/token";
            $this->HW_PUSH_SERVER = "https://push-api.cloud.huawei.com/v1/{$huaweiPushConfig['APP_ID']}/messages:send";
            $this->HW_DEFAULT_LOG_LEVEL = 3;
        }
    }
